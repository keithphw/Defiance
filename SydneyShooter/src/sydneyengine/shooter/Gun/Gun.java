...
public abstract class Gun extends SSAdapter implements Item {
	...
	public void render(ViewPane viewPane) {
		...
		if (reloadClip == false){
			...
			float clipFullnessAfterReload = ((float)ammoToPutIn + ammoInCurrentClip)/maxAmmoInClip;
		...
		*/
		...
		if (proportionOfAmmoInClip != 0) {
			...
			*/
			...
			g.setColor(new Color(readyToFireColor.getRed(), readyToFireColor.getGreen(), readyToFireColor.getBlue(), 50));
			
			int radius= 120;
			int width = 20;
			//int numClips = (int)Math.ceil((float)ammo/maxAmmoInClip);	
			g.setStroke(new BasicStroke(width));	
			g.drawArc(viewPane.getWidth()/2 - radius, viewPane.getHeight()/2 - radius, 2 * radius, 2 * radius, 90, 
					(int) (proportionOfAmmoInClip * 360));
			g.setStroke(orig);
		...
		*/
		...
		float StringWidth = fm.stringWidth(this.getName());
		...
		g.drawString(this.getName() + "  [ " + numClipsString + " ]", xCoordBetweenBars - StringWidth - 30, 
				//nameYCoord
				60);
	...
	public void fire(double secondsLeft, double timeAtStartOfMoveSeconds, Player player, float xPosWhenFired, float yPosWhenFired, float angle, 
			...
			Bullet bullet = createBullet(player, xPosWhenFired, yPosWhenFired, angle, 
					player.getMouseTargetX() + player.getX(), player.getMouseTargetY() + player.getY(), // don't ask why, it just works
					timeAtStartOfMoveSeconds, playerSpeedX, playerSpeedY);
